...
const textExtensions = new Set([
  ".c", ".cc", ".conf", ".cpp", ".css", ".go", ".h", ".hpp", ".html", ".ini",
...
function isTextCandidate(path) {
  return path === ".gitignore" || path === "Dockerfile" || path === "Procfile" || textExtensions.has(extension(path));
}
...
async function walk(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (info.isSymbolicLink()) {
      output.push({ path, absolutePath, info, symlink: true });
    } else if (info.isDirectory()) {
      output.push(...await walk(absolutePath));
    } else if (info.isFile()) {
      output.push({ path, absolutePath, info, symlink: false });
    }
  }
  return output;
}
...
for (const file of files) {
  ...
  if (symlink) {
    findings.push(`${path}: symbolic links are forbidden in the hardened baseline`);
    continue;
  }

  if (info.size > MAX_FILE_BYTES) {
    findings.push(`${path}: file exceeds ${MAX_FILE_BYTES} byte repository safety limit`);
  }
  ...
  if (buffer.includes(0)) {
    findings.push(`${path}: unexpected NUL/binary content in text file`);
    continue;
  }
  ...
  for (const [label, pattern] of riskyCodePatterns) {
    if (pattern.test(text)) findings.push(`${path}: possible ${label}`);
  }
}
